...
if __name__ == '__main__':
    import reproducelayout
    import reproduce_layout_GUI
else:
    from . import reproducelayout
    from . import reproduce_layout_GUI
...
class reproduceLayoutDialog(reproduce_layout_GUI.reproduceLayoutGUI):
    ...
    def __init__(self, parent, replicator, mod_ref, logger):
        reproduce_layout_GUI.reproduceLayoutGUI.__init__(self, parent)
    ...
    def level_changed(self, event):
        ...
        list_sheetsChoices = self.replicator.get_sheets_to_reproduce(self.src_anchor_module, self.src_anchor_module.sheet_id[index])
    ...
    def OnOk(self, event):
        ...
        # grab checkboxes
        reproduce_containing_only = not self.chkbox_intersecting.GetValue()
        ...
        sheets_on_a_level = self.replicator.get_sheets_to_reproduce(self.src_anchor_module, self.src_anchor_module.sheet_id[level])
        ...
        try:
            ...
            self.replicator.reproduce_layout(self.src_anchor_module, self.src_anchor_module.sheet_id[0:level + 1], dst_sheets,
                                             containing=reproduce_containing_only,
            ...
            message = "Fatal error when running replicator.\n"\
                    ...
                    + "Please attach the reproduce_layout.log which you should find in the project folder."
            dlg = wx.MessageDialog(self, message, caption, wx.OK | wx.ICON_ERROR)
            dlg.ShowModal()
            dlg.Destroy()
            logging.shutdown()
...
class reproduceLayout(pcbnew.ActionPlugin):
    """
    ...
    def defaults(self):
        self.name = "reproduce layout"
        self.category = "Modify Drawing PCB"
        self.description = "reproduce layout of a hierchical sheet"
        self.icon_file_name = os.path.join(
                os.path.dirname(__file__), 'duplicate-reproduce_layout.svg.png')
    ...
    def Run(self):
        ...
        logger.info("reproduce layout plugin version: " + VERSION + " started")
        ...
        try:
            replicator = reproducelayout.Replicator(board)
        except LookupError as exception:
            caption = 'reproduce Layout'
            message = str(exception)
        ...
        dlg = reproduceLayoutDialog(_pcbnew_frame, replicator, src_anchor_mod_reference, logger)
