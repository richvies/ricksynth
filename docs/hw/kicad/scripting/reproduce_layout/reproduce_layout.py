...
sys.path.append('/Users/richarddavies/Desktop/kicad/scripting/reproduce_layout')
...
class Reproducer():
    ...
    def get_sheet_id(self, module):
        """ get sheet id """
        module_path = get_path(module).split('/')
        print('get path '+ repr(get_path(module)))
    ...
    def get_sheets_to_reproduce(self, mod, level):
        ...
        logger.debug("All sheets to reproduce:\n" + repr(all_sheets))

        # remove duplicates
        all_sheets.sort()
        all_sheets = list(k for k, _ in itertools.groupby(all_sheets))
        logger.debug("All sheets to reproduce sorted:\n" + repr(all_sheets))

        # remove source sheet
        if src_sheet_path in all_sheets:
            index = all_sheets.index(src_sheet_path)
            del all_sheets[index]
        logger.info("All sheets to reproduce sorted and without source sheet:\n" + repr(all_sheets))

        return all_sheets
    
    # changed to only find modules on specific subsheet i.e. does not recurse to further subsheets
    def get_modules_on_sheet(self, level):
        logger.info('get modules on sheet ' + repr(level))
        modules_on_sheet = []
        # level_depth = len(level)
        for mod in self.modules:
            # if level == mod.sheet_id[0:level_depth]:
            if level == mod.sheet_id[-1]:
                modules_on_sheet.append(mod)
        logger.info(repr(modules_on_sheet))
    ...
    def reproduce_modules(self):
        ...
        for st_index in range(nr_sheets):
            ...
            for mod_index in range(nr_mods):
                ...
                # reproduce also text layout - also for anchor module. I am counting that the user is lazy and will
    ...
    def reproduce_zones(self):
        """ method which reproduces zones"""
        ...
        for index in range(len(self.dst_sheets)):
            ...
            for track in tracks_on_nets_on_sheet:
                ...
                if track not in self.src_tracks:
                    ...
                    # remove the tracks containing/interesecting in the reproduced bounding box
                    if containing:
            ...
            for zone in all_zones:
                ...
                if zone not in self.src_zones:
                    ...
                    # remove the zones containing/interesecting in the reproduced bounding box
                    if containing:
            ...
            for drawing in self.board.GetDrawings():
                ...
                # add text in/intersecting with the reproduced bounding box to
    ...
    def reproduce_layout(self, src_anchor_module, level, dst_sheets,
...
def test_file(in_filename, out_filename, src_anchor_module_reference, level, sheets, containing, remove):
    ...
    # get list of sheets
    sheet_list = replicator.get_sheets_to_reproduce(src_anchor_module, src_anchor_module.sheet_id[index])
    ...
    replicator.reproduce_layout(src_anchor_module, src_anchor_module.sheet_id[0:index+1], dst_sheets,
...
def main():
    ...
    file_handler = logging.FileHandler(filename='reproce_layout.log', mode='w')
    ...
    logger.info("Reproduce layout plugin version: " + VERSION + " started in standalone mode")

    os.chdir(os.path.join(os.path.dirname(os.path.realpath(__file__)), "first_attempt"))
    
    logger.info("Testing reproduce")
    input_file = '/Users/richarddavies/Desktop/kicad/test/test.kicad_pcb'
    output_file = '/Users/richarddavies/Desktop/kicad/test/test_out.kicad_pcb'

    # load file
    src_board = pcbnew.LoadBoard(input_file)
    reproducer = Reproducer(src_board)

    # find all subsheets
    logger.info("get all top level subsheets")
    list_of_subsheets = []
    for sheet_id in reproducer.dict_of_sheets:
        sheet_name = reproducer.dict_of_sheets[sheet_id][0]
        ref_sch = reproducer.dict_of_sheets[sheet_id][1]
        ref_pcb = os.path.join(reproducer.project_folder, ref_sch.replace('.sch', '.kicad_pcb'))
        logger.info("sheet id " + sheet_id + " name " + sheet_name + " ref  sch " + ref_sch + " ref pcb " + ref_pcb)

        list_of_subsheets.append([sheet_id, sheet_name, ref_sch, ref_pcb])

    list_of_subsheets = sorted(list_of_subsheets, key=lambda x:os.path.exists(x[3]), reverse=True)

    # signal which do and dont have assoc .kicad_pcb file
    logger.info("search for associated layout files")
    list_of_subsheets_to_reproduce = []
    for subsheet in list_of_subsheets:
        pcb_file = subsheet[3]
        if os.path.exists(pcb_file):
            logger.info('found ' + pcb_file)
            list_of_subsheets_to_reproduce.append(subsheet)
        else:
            logger.info('not found ' + pcb_file)

    logger.info("files to reproduce" + repr(list_of_subsheets_to_reproduce))

    for module in reproducer.modules:
        print(module.sheet_id)

    logger.info("Reproduce modules")
    curr_pos = [10000000, 250000000]
    for [sheet_id, sheet_name, ref_sch, ref_pcb] in list_of_subsheets_to_reproduce:
        logger.info("Reproduce " + repr(sheet_name) + " from " + repr(ref_pcb))

        # src modules to move
        src_modules = reproducer.get_modules_on_sheet(sheet_name)

        # ref files
        ref_board = pcbnew.LoadBoard(ref_pcb)
        ref_reproducer = Reproducer(ref_board)

        # ref objects to copy
        ref_modules = ref_reproducer.modules
        ref_tracks = ref_board.GetTracks()

        # get all the tracks for reproduction
        ...
        # ref positioning
        ref_box = ref_reproducer.get_modules_bounding_box(ref_reproducer.modules)
        ref_width = ref_box.GetWidth()
        ref_height = ref_box.GetHeight()
        ref_start_pos = (ref_box.GetLeft(), ref_box.GetTop())
        move_vector = tuple(curr_pos[x] - ref_start_pos[x] for x in range(2))
        move_vector = pcbnew.wxPoint(*move_vector)
        logger.info("subpcb width " + str(ref_width))
        logger.info("subpcb height " + str(ref_height))
        logger.info("move vector " + str(move_vector))

        
        # copy positions
        logger.info("Copy posistions")
        for src_module in src_modules:
            logger.info("try " + module.ref)
            for ref_module in ref_modules:
                if (ref_module.ref[0] == src_module.ref[0]) and (ref_module.ref[2:] == src_module.ref[2:]):
                    logger.info("Found" + ref_module.ref)

                    # update position
                    new_pos = [ref_module.mod.GetPosition()[x] + move_vector[x] for x in range(2)]
                    src_module.mod.SetPosition(pcbnew.wxPoint(*new_pos))

                    logger.info("new pos " + repr(src_module.mod.GetPosition()))
                    logger.info("new orient " + repr(src_module.mod.GetOrientationDegrees()))

                    break

        # copy tracks
        logger.info("Copy tracks")
        for track in ref_tracks:
            logger.info("try " + track.GetNetname() + " " + str(track.GetNet().GetNet()))
            # make a duplicate, move it, select proper net and add it to the board
            # kicad seems to rename tracks correctly based on pads connected
            new_track = track.Duplicate()
            new_track.Move(move_vector)
            # new_track.SetNetCode(to_net_code)
            # new_track.SetNet(to_net_item)
            src_board.Add(new_track)

        # update x posistion for placing next subpcb
        curr_pos[0] += ref_width + 20000000
    ...
    pcbnew.SaveBoard(output_file, src_board)
    pcbnew.Refresh()
